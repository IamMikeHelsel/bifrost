...
class IndustrialDataSimulator:
    ...
    async def setup_nodes(self, server: Server):
        ...
        device_folder = await objects.add_folder("ns=2;s=Factory", "Factory Floor")
        
        # Temperature sensors
        temp_folder = await device_folder.add_folder("ns=2;s=Temperature", "Temperature Sensors")
        for i in range(5):
        ...
        # Pressure sensors
        pressure_folder = await device_folder.add_folder("ns=2;s=Pressure", "Pressure Sensors")
        for i in range(3):
        ...
        # Flow meters
        flow_folder = await device_folder.add_folder("ns=2;s=Flow", "Flow Meters")
        for i in range(3):
        ...
        control_folder = await device_folder.add_folder("ns=2;s=Control", "Control Setpoints")
        for i in range(3):
        ...
        # Status indicators
        status_folder = await device_folder.add_folder("ns=2;s=Status", "Status Indicators")
        for i in range(5):
        ...
        # System information
        info_folder = await device_folder.add_folder("ns=2;s=System", "System Information")
    ...
    async def update_data(self):
        ...
        for i in range(5):
            ...
            variation = 3.0 * math.sin(elapsed / 60.0 + i) + random.uniform(-1.0, 1.0)
        ...
        for i in range(3):
            ...
            variation = 0.05 * math.sin(elapsed / 30.0 + i) + random.uniform(-0.02, 0.02)
        ...
        for i in range(3):
            ...
            variation = 10.0 * math.sin(elapsed / 45.0 + i) + random.uniform(-5.0, 5.0)
        ...
        if random.random() < 0.1:  # 10% chance to change each second
...
async def main():
    ...
    parser.add_argument("--namespace", default="http://bifrost.industrial.example",

...
from pyModbusTCP.constants import EXP_ILLEGAL_FUNCTION, EXP_ILLEGAL_DATA_ADDRESS, EXP_ILLEGAL_DATA_VALUE
...
class SimulatedDevice:
    ...
    def _update_sensors(self):
        ...
        for i in range(10):
            ...
            daily_cycle = 5.0 * math.sin((current_time % 86400) / 86400 * 2 * math.pi)
...
class BifrostModbusServer:
    ...
    def _setup_data_hooks(self):
        ...
        original_get_holding_registers = self.server.data_bank.get_holding_registers
        original_set_holding_registers = self.server.data_bank.set_holding_registers
        
        def custom_get_holding_registers(address: int, nb_reg: int) -> Optional[List[int]]:
            ...
            if random.random() < self.error_rate:
                ...
                logging.warning(f"Simulated communication error for address {address}")
                return None
            ...
            try:
                ...
                logging.error(f"Error reading registers {address}-{address+nb_reg-1}: {e}")
                self.stats["requests_failed"] += 1
                return None
                
        def custom_set_holding_registers(address: int, values: List[int]) -> bool:
            ...
            if address < 30:  # Sensor data is read-only
                logging.warning(f"Attempt to write to read-only address {address}")
                self.stats["requests_failed"] += 1
                return False
        ...
        self.server.data_bank.get_holding_registers = custom_get_holding_registers
        self.server.data_bank.set_holding_registers = custom_set_holding_registers
        
    def set_error_simulation(self, error_rate: float, response_delay: float = 0.0):
        ...
        logging.info(f"Error simulation: {self.error_rate*100:.1f}% error rate, "
    ...
    def get_statistics(self) -> Dict:
        ...
        if self.stats["requests_total"] > 0:
            success_rate = self.stats["requests_successful"] / self.stats["requests_total"]
        ...
        return {
            ...
            "requests_per_second": self.stats["requests_total"] / max(uptime, 1.0)
        }
    ...
    def start(self):
        ...
        try:
            ...
            # Start Modbus server
            self.server.start()
            logging.info(f"Modbus TCP server started on {self.host}:{self.port}")
            ...
            try:
                while True:
                    ...
                    if self.stats["requests_total"] % 100 == 0 and self.stats["requests_total"] > 0:
                        stats = self.get_statistics()
                        logging.info(f"Stats: {stats['requests_total']} requests, "
...
def main():
    ...
    # Start server
    logging.info(f"Starting Bifrost Modbus TCP Simulator")
    logging.info(f"Configuration: host={args.host}, port={args.port}, "

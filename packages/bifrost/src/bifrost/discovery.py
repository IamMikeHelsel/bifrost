...
from bifrost_core import DeviceInfo, ProtocolType, emit_event
...
class NetworkDiscovery:
        ...
        for device in self.discovered_devices.values():
            if device.ip_address:
                devices.append(DeviceInfo(
                    ...
                    port=502 if ProtocolType.MODBUS_TCP in (device.protocol or []) else None,
    ...
    async def _modbus_discovery(self, network: str, timeout: float) -> None:
        ...
        async def check_modbus(ip: str, port: int) -> None:
            try:
                ...
                await self._add_discovered_device(
                    f"unknown_{ip}_{port}", ip, None, "modbus",
        ...
        await asyncio.gather(*[check_with_semaphore(task) for task in tasks], 
    ...
    async def _probe_host(self, ip: str) -> None:
        ...
        try:
            ...
            ports = []
            common_ports = [22, 23, 80, 443, 502, 503, 505, 4840]  # SSH, Telnet, HTTP, HTTPS, Modbus, OPC UA
            ...
            await self._add_discovered_device(
                f"unknown_{ip}", ip, hostname, "probe",
    ...
    def _create_bootp_request(self, xid: int) -> bytes:
        ...
        packet = struct.pack(
            ...
            0,  # flags
            0,  # ciaddr
            0, 0, 0, 0,  # yiaddr, siaddr, giaddr (all 0)
    ...
    def _guess_device_type(self, hostname: Optional[str], ports: List[int]) -> Optional[str]:

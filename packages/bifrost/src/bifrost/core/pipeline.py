...
class Pipeline(ABC):
    ...
    @abstractmethod
    async def process(self, data: DataPoint) -> DataPoint | None:
        """Process a single data point through the pipeline."""

    async def add_processor(
        self, processor: Callable[[DataPoint], DataPoint]
    ) -> None:
    ...
    async def pull(self) -> DataPoint | None:
...
class FilterPipeline(Pipeline):
    ...
    def __init__(
        self, filter_func: Callable[[DataPoint], bool], buffer_size: int = 1000
    ) -> None:
        super().__init__(buffer_size)
...
class AggregationPipeline(WindowedPipeline):
    ...
    async def _process_window(
        ...
        # Get numeric values
        values = [
            dp.value for dp in window if isinstance(dp.value, (int, float))
        ]
        if not values:
            return None
        ...
        if self.aggregation_type == "mean":
            ...
            raise ValueError(
                f"Unknown aggregation type: {self.aggregation_type}"
            )

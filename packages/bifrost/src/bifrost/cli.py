...
from .discovery import discover_devices, assign_device_ip
...
def discover(
    ...
    timeout: float = typer.Option(
        ...
        "--timeout",
        "-t",
        help="⏱️ Timeout per method (seconds)"
    ),
    ...
    async def run_discovery():
        with Progress(
            ...
            task = progress.add_task(
                f"[cyan]Discovering devices on {network}...", 
                total=None
            )
            ...
            try:
                ...
                console.print(f"✅ Found [bold green]{len(devices)}[/bold green] devices")
    ...
    async def run_assignment():
        with Progress(
            ...
            try:
                ...
                if success:
                    console.print(f"✅ IP assignment successful", style="green")
...
def connect(
    ...
    async def run_connection():
        ...
        with Progress(
            ...
            task = progress.add_task(
                f"[cyan]Connecting to {host}:{port}...", 
                total=None
            )
...
def scan(
    ...
    async def run_scan():
        with Progress(
            ...
            task = progress.add_task(
                f"[cyan]Scanning {network} on ports {ports}...", 
                total=None
            )
...
def status():
    ...
    console.print(Panel.fit(
        "🌉 [bold blue]Bifrost System Status[/bold blue]",
        border_style="blue"
    ))
...
async def _interactive_mode(connection) -> None:
    ...
    console.print("Commands: read <address> [count], write <address> <value>, health, quit")

...
class TestEndToEndModbusWorkflow:
    ...
    async def test_complete_modbus_workflow(self):
        ...
        with patch("bifrost.discovery.NetworkDiscovery") as mock_discovery_class, \
            ...
            tags = [
                Tag("temperature", "40001", DataType.INT16, description="Temperature sensor"),
                Tag("pressure", "40002", DataType.INT16, description="Pressure sensor"),
                Tag("flow_rate", "40003", DataType.INT16, description="Flow rate sensor"),
            ]
...
class TestEventSystemIntegration:
    ...
    async def test_event_driven_monitoring(self):
        ...
        from bifrost_core import EventBus, ConnectionStateEvent, DataReceivedEvent, ErrorEvent
        ...
        with patch("pymodbus.client.AsyncModbusTcpClient") as mock_client_class:
            ...
            connection_events = [e for e in events_received if isinstance(e, ConnectionStateEvent)]
            data_events = [e for e in events_received if isinstance(e, DataReceivedEvent)]
...
class TestRealWorldScenarios:
    ...
    async def test_factory_monitoring_scenario(self):
        ...
        device_configs = [
            {"host": "192.168.1.100", "name": "Production Line 1 PLC", "tags": ["temp_1", "pressure_1"]},
            {"host": "192.168.1.101", "name": "Production Line 2 PLC", "tags": ["temp_2", "flow_rate"]},
            {"host": "192.168.1.102", "name": "Quality Control PLC", "tags": ["weight", "dimension"]},
        ]
        ...
        with patch("pymodbus.client.AsyncModbusTcpClient") as mock_client_class:
            ...
            for i, config in enumerate(device_configs):
                ...
                mock_response = MagicMock()
                mock_response.isError.return_value = False
                mock_response.registers = [20 + i * 10, 30 + i * 10]  # Different values per device
                mock_client.read_holding_registers.return_value = mock_response
    ...
    async def test_device_failover_scenario(self):
        ...
        with patch("pymodbus.client.AsyncModbusTcpClient") as mock_client_class:
            ...
            primary_client.connect.side_effect = ConnectionError("Primary device failed")

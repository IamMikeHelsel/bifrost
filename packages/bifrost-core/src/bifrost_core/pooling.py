...
from .events import ErrorEvent, emit_event
...
class ConnectionPool:
    ...
    async def get_connection(
        self, connection_factory, connection_key: str, **factory_kwargs
    ) -> BaseConnection:
    ...
    async def connection(
        ...
        conn = await self.get_connection(
            connection_factory, connection_key, **factory_kwargs
        )
        try:
            yield conn
        finally:
    ...
    def _get_available_connection(self) -> PooledConnection | None:
    ...
    def _return_connection(self, pooled_conn: PooledConnection) -> None:
        ...
        if pooled_conn in self._borrowed:
            ...
            if (
                pooled_conn.connection.is_connected
                and pooled_conn.age < self.max_lifetime
                and pooled_conn.idle_time < self.max_idle_time
            ):
    ...
    async def _cleanup_stale_connections(self) -> None:
        ...
        for pooled_conn in self._available.copy():
            if (
                pooled_conn.age > self.max_lifetime
                or pooled_conn.idle_time > self.max_idle_time
                or not pooled_conn.connection.is_connected
            ):
...
async def pooled_connection(
    connection_factory, connection_key: str, **factory_kwargs
) -> AsyncIterator[BaseConnection]:

    ...
    for i in range(iterations):
        ...
        try:
            ...
            if not result.isError():
                success_count += 1
                latencies.append((read_end - read_start) * 1000000)  # microseconds
...
def test_concurrent_reads(host='127.0.0.1', port=502, num_threads=10, reads_per_thread=100):
    """Test concurrent read performance."""
    print(f"Python Concurrent Read Test ({num_threads} threads, {reads_per_thread} reads each)...")
...
def test_write_operations(host='127.0.0.1', port=502, iterations=100):
    ...
    for i in range(iterations):
        ...
        try:
            ...
            if not result.isError():
                success_count += 1
                write_latencies.append((write_end - write_start) * 1000000)  # microseconds
...
def main():
    ...
    # Concurrent reads
    concurrent_result = test_concurrent_reads(num_threads=10, reads_per_thread=100)
    ...
    if sequential_result:
        ...
        print(f"{'Sequential Reads (ops/s)':<30} {python_rps:<15.0f} {go_sequential_rps:<15.0f} {rps_improvement:<15.1f}x")
        print(f"{'Average Latency (µs)':<30} {python_latency:<15.1f} {go_avg_latency_us:<15.1f} {latency_improvement:<15.1f}x")
    ...
    if write_result:
        ...
        print(f"{'Write Ops (ops/s)':<30} {python_writes:<15.0f} {'N/A':<15} {'N/A':<15}")

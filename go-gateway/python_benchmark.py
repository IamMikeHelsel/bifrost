...
class PythonModbusBenchmark:
    ...
    def benchmark_sequential_reads(self, iterations=1000):
        ...
        if success_count > 0:
            ...
            print(
                f"   ✅ Sequential reads: {success_count}/{iterations} successful"
            )
    ...
    def benchmark_concurrent_reads(self, num_threads=10, reads_per_thread=100):
        ...
        if success_count > 0:
            ...
            print(
                f"   ✅ Concurrent operations: {success_count}/{total_operations} successful"
            )
            print(
                f"   ✅ Performance: {ops_per_second:.0f} ops/second with {num_threads} threads"
            )
            print(f"   ✅ Average latency: {avg_latency:.3f}ms")
            print(f"   ✅ Median latency: {median_latency:.3f}ms")
            print(f"   ✅ Total time: {total_time:.3f}s")
    ...
    def benchmark_read_write_operations(self, iterations=100):
        ...
        for i in range(iterations):
            ...
            write_success = self.write_holding_register(
        ...
        if success_count > 0:
            ops_per_second = (
                success_count * 2
            ) / total_time  # 2 ops per iteration (write + read)
            ...
            print(
                f"   ✅ Read/Write operations: {success_count}/{iterations} successful"
            )
        ...
        if python_results:
            ...
            for result in python_results:
                if result["type"] == "sequential_reads":
                    ...
                    print(
                        f"{'Sequential Reads (ops/s)':<30} {python_rps:<15.0f} {go_sequential_rps:<15.0f} {improvement:<15.1f}x"
                    )
                    ...
                    print(
                        f"{'Average Latency (ms)':<30} {python_latency_ms:<15.3f} {go_latency_ms:<15.3f} {latency_improvement:<15.1f}x"
                    )
